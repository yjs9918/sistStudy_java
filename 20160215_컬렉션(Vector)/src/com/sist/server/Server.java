...
public class Server implements Runnable{
	...
	public void run()
	{
		...
		while(true)
		{
			...
			{
				// 클라이언트의 정보 => ip,port(Socket)
				Socket s=ss.accept(); //accept() => 반응이 와야 작동함 (ex:접속을 해야 다음문장 수행) 리턴형 : Socket
				//s => client
	...
	class ClientThread extends Thread //통신은 여기서 이루어진다
	{
		String id,name,sex,pos;
		Socket s;
		BufferedReader in; // client요청값을 읽어온다
		OutputStream out; // client로 결과값을 응답할 때 
		//Stream:byte 단위 전송      / Reader:2byte 단위 전송
		...
		//개인적으로
		public synchronized void messageTo(String msg) //동기화
		{
			try
			{
				out.write((msg+"\n").getBytes()); // \n : 명령이 끝났다
